...
def _related(organization_id: uuid.UUID) -> dict[str, list[str]]:
    return {"organization_ids": [str(organization_id)]}
    ...
    *,
    ...
    contract = Contract(
        ...
        original_end_date=(
            date(2026, 9, 30)
            if status
            not in {ContractStatus.DRAFT, ContractStatus.APPROVAL}
            else None
        ),
...
def _addendum(
    ...
    *,
    ...
    number: str,
    status: ContractAddendumStatus = ContractAddendumStatus.APPROVAL,
...
def test_addendum_signing_rejects_currency_mismatch_without_mutation(
    ...
    customer = _organization(db_session, "Currency Org")
...
def test_deadline_shortening_does_not_require_extension_reason(
    ...
    customer = _organization(db_session, "Shortening Org")
    actor_id = _actor_id(test_user)
    actor = db_session.get(User, actor_id)
    assert actor is not None
    contract = _contract(
...
def test_addendum_cannot_sign_after_parent_becomes_terminal(
    ...
    customer = _organization(db_session, "Terminal Parent Org")
    actor_id = _actor_id(test_user)
    actor = db_session.get(User, actor_id)
    assert actor is not None
    contract = _contract(
...
def _configure_scope_target(
    ...
    *,
    ...
    scope_config = _related(organization.id) if scope_type == ScopeType.RELATED else None
    _grant(
        db,
        user=user,
    ...
    creator = user if scope_type == ScopeType.OWN else _user(
        db, f"edge-creator-{scope_type.value}-{suffix}"
    )
    contract = _contract(
        db,
        customer=organization,
...
def test_status_command_honors_all_contract_scope_types(
    ...
    user, contract = _configure_scope_target(
        db_session,
        scope_type=scope_type,
...
def test_cp42_migration_downgrade_upgrade_round_trip() -> None:
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            assert version == "0011_stage4_contracts_core"
        engine.dispose()
        ...
        with engine.connect() as connection:
            ...
            version = connection.scalar(text("SELECT version_num FROM alembic_version"))
            assert version == "0012_stage4_contract_lifecycle"
        engine.dispose()
    finally:
