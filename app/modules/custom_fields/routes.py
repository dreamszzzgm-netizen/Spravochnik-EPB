...
_dep_manage = Depends(require_scoped_permission("custom_fields.manage"))  # noqa: B008
...
def _parent_or_404(
    ...
    *,
    ...
    if entity_type == "opo":
        ...
        allowed = (
            entity is not None
            and can_access_technical_device(authorization, entity)
        )
    elif entity_type == "building":
        entity = get_building(db, entity_id)
        allowed = (
            entity is not None
            and can_access_building(authorization, entity)
        )
    else:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
...
def read_values(
    entity_type: str,
    entity_id: uuid.UUID,
    authorization: AuthorizationContext = _dep_manage,
    db: Session = Depends(get_db),
):
    _parent_or_404(
        db,
