...
class TechnicalDeviceService:
    ...
    def update_technical_device(
        ...
        *,
        ...
        if organization_id_provided and organization_id is None:
            raise TechnicalDeviceNotFoundError("organization_id cannot be set to None")

        if organization_id_provided and organization_id is not None:
            org = get_organization(db, organization_id)
            if org is None:
                raise TechnicalDeviceNotFoundError("Organization not found")
            if org.deleted_at is not None:
        ...
        if (
            ...
            final_opo = get_opo(db, final_opo_id)
            if (
                final_opo is not None
                and final_opo.deleted_at is None
                and final_opo.owner_organization_id != final_organization_id
                and final_opo.operating_organization_id != final_organization_id
            ):
                raise TechnicalDeviceNotFoundError(
                    "OPO does not belong to this organization"
                )
